...
router.get("/", async (req, res) => {
  try {
    const [results] = await pool.query(
      "SELECT Title, Description, Image FROM web_development LIMIT 1",
    );

    if (results.length === 0) {
      return res
        .status(404)
        .json({ message: "Web Development content not found" });
    }
...
router.get("/edit", async (req, res) => {
  try {
    const [results] = await pool.query("SELECT * FROM web_development LIMIT 1");
